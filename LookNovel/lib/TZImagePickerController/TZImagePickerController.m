...
@interface TZAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate,UIViewControllerAnimatedTransitioning,UIViewControllerTransitioningDelegate> {
...
//    }
    _tableView.frame = CGRectMake(0, 0, self.view.tz_width, self.view.tz_height);
}
...
//    [self.navigationController pushViewController:photoPickerVc animated:YES];
    [self dismissViewControllerAnimated:YES completion:^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ALBUM_DID_DISSMISS" object:nil userInfo:@{@"albumModel":model,@"columnNumber":@(self.columnNumber)}];
    }];
}
...
- (void)dismiss{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ALBUM_DID_DISSMISS" object:nil userInfo:nil];
    }];
}
...
- (void)presentAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    coverView.frame = CGRectMake(0, 64 + kIPhoneX_TOP_HEIGHT, containerView.width, containerView.height - (64 + kIPhoneX_TOP_HEIGHT));
    ...
    toVC.view.frame = CGRectMake(0, -[self selfHeight] + 64 + kIPhoneX_TOP_HEIGHT, CGRectGetWidth(containerView.frame), [self selfHeight]);
    //开始动画吧
    [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
        toVC.view.transform = CGAffineTransformMakeTranslation(0, CGRectGetHeight(toVC.view.frame));
        coverView.alpha = 1;
...
- (void)dismissAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    UIView *tempView = subviewsArray.lastObject;
    UIView *coverView = [subviewsArray objectAtIndex:1];
    [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{

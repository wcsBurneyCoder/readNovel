...
@interface TZCompleteBtn : UIView
@property (nonatomic, weak) UIImageView *numberImageView;
@property (nonatomic, weak) UILabel *numberLabel;
@property (nonatomic, weak) UIButton *doneBtn;

@end

@implementation TZCompleteBtn
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [doneButton setTitleColor:UIColorHex([LNSkinHelper sharedHelper].currentSkin.appMainColor) forState:UIControlStateNormal];
        [doneButton setTitleColor:UIColorHex(@"CCCCCC") forState:UIControlStateDisabled];
        [self addSubview:doneButton];
        self.doneBtn = doneButton;
        
        
        UIImageView *numberImageView = [[UIImageView alloc] initWithImage:[UIImage tz_imageNamedFromMyBundle:@"photo_number_icon"]];
        numberImageView.clipsToBounds = YES;
...
- (void)layoutSubviews
{
    ...
    self.doneBtn.frame = CGRectMake(0, 0, 35, self.height);
    self.numberImageView.frame = CGRectMake(32, 0, 18, 18);
    self.numberLabel.frame = self.numberImageView.frame;
}

@end

@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,UITableViewDelegate,UITableViewDataSource,TOCropViewControllerDelegate> {
...
/**选择相册列表*/
@property (nonatomic, strong) UIView *selectAlbumsView;
/**选择相册列表时的遮罩*/
...
/******/
- (UIView *)selectAlbumView
{
    UIView *baseView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 44)];
    [baseView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickAlbumSelect)]];
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage tz_imageNamedFromMyBundle:@"photo_arrow_down"]];
...
- (void)layoutTitleView
{
    ...
    if (titleView) {
        UIView *contentView = self.navigationItem.titleView.subviews.firstObject;
        UILabel *label = (UILabel *)contentView.subviews.firstObject;
        UIImageView *imageView = (UIImageView *)contentView.subviews.lastObject;
        label.width = [label.text widthForFont:label.font];
        label.height = 44;
        label.left = 0;
        label.top = 0;
        imageView.width = imageView.image.size.width;
        imageView.height = 44;
        imageView.left = label.width + 7;
        imageView.top = 0;
        titleView.frame = CGRectMake(0, 0, imageView.right, 44);
    }
}

- (void)clickAlbumSelect
{
    _albumIsOpen = !_albumIsOpen;
    if (_albumIsOpen) {
        [self showAlbumsView];
    }
    else
        [self dismissAlbumsView];
}

- (UIView *)rightItemView
{
    _completeBtn = [[TZCompleteBtn alloc] initWithFrame:CGRectMake(0, 0, 50, 44)];
    
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    _doneButton = _completeBtn.doneBtn;
    [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
    _doneButton.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
    
    _numberImageView = _completeBtn.numberImageView;
    _numberImageView.hidden = tzImagePickerVc.selectedModels.count <= 0;
    _numberImageView.clipsToBounds = YES;

    _numberLabel = _completeBtn.numberLabel;
...
- (UIView *)selectAlbumsView
{
    if (_selectAlbumsView) {
        [self.view addSubview:_selectAlbumsView];
        return _selectAlbumsView;
    }
    UIView *selectAlbumsView = [[UIView alloc] initWithFrame:CGRectMake(0, 64 + kIPhoneX_TOP_HEIGHT, self.view.width, 0)];
    selectAlbumsView.clipsToBounds = YES;
    ...
    [selectAlbumsView addSubview:tableView];
    [tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(selectAlbumsView);
    }];
    [self.view addSubview:selectAlbumsView];
    selectAlbumsView.hidden = YES;
    _selectAlbumsView = selectAlbumsView;
    return _selectAlbumsView;
}
...
- (UIControl *)coverView
{
    ...
    coverView.backgroundColor = [UIColorHex(@"000000") colorWithAlphaComponent:0.4];
    [coverView addTarget:self action:@selector(dismissAlbumsView) forControlEvents:UIControlEventTouchUpInside];
...
- (void)showAlbumsView
{
    CGFloat viewHeight = MIN(_albumsArray.count * 75, kScreenHeight * 0.4);
    self.coverView.alpha = 1;
    self.coverView.hidden = NO;
    self.selectAlbumsView.hidden = NO;
    self.navigationController.view.userInteractionEnabled = NO;
    [UIView animateWithDuration:0.25 animations:^{
        self.coverView.alpha = 1;
        self.selectAlbumsView.height = viewHeight;
    ...
    if (titleView) {
        UIView *contentView = self.navigationItem.titleView.subviews.firstObject;
        UIImageView *imageView = (UIImageView *)contentView.subviews.lastObject;
        imageView.image = [UIImage tz_imageNamedFromMyBundle:@"photo_arrow_up"];
    }
}

- (void)dismissAlbumsView
{
    _albumIsOpen = NO;
    self.navigationController.view.userInteractionEnabled = NO;
    [UIView animateWithDuration:0.25 animations:^{
        self.coverView.alpha = 0;
        self.selectAlbumsView.height = 0;
    } completion:^(BOOL finished) {
        [self.coverView removeFromSuperview];
        [self.selectAlbumsView removeFromSuperview];
        self.navigationController.view.userInteractionEnabled = YES;
    }];
    UIView *titleView = self.navigationItem.titleView;
    if (titleView) {
        UIView *contentView = self.navigationItem.titleView.subviews.firstObject;
        UIImageView *imageView = (UIImageView *)contentView.subviews.lastObject;
        imageView.image = [UIImage tz_imageNamedFromMyBundle:@"photo_arrow_down"];
    }
}

- (void)didSelectAlbum:(TZAlbumModel *)model
{
    _albumIsOpen = NO;
    UIView *titleView = self.navigationItem.titleView;
    if (titleView) {
        UIView *contentView = self.navigationItem.titleView.subviews.firstObject;
        UIImageView *imageView = (UIImageView *)contentView.subviews.lastObject;
        imageView.image = [UIImage tz_imageNamedFromMyBundle:_albumIsOpen?@"photo_arrow_up":@"photo_arrow_down"];
        if (model) {
            UILabel *label = (UILabel *)contentView.subviews.firstObject;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.model = _albumsArray[indexPath.row];
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    TZAlbumModel *model = _albumsArray[indexPath.row];
...
//    self.navigationItem.titleView = [self selectAlbumView];
//    [self layoutTitleView];
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"icon_return_8x14_"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(navLeftBarButtonClick)];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<TZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumsArray = models;
                for (TZAlbumModel *albumModel in self->_albumsArray) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        if (tzImagePickerVc.allowCrop && tzImagePickerVc.maxImagesCount == 1) {
            [tzImagePickerVc showProgressHUD];
            [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                if (isDegraded == NO) {
                    [tzImagePickerVc hideProgressHUD];
                    TOCropViewController *cropVc = [[TOCropViewController alloc] initWithImage:photo];
                    cropVc.delegate = self;
                    cropVc.asset = model.asset;
                    [cropVc setResetAspectRatioEnabled:NO];
                    [cropVc setAspectRatioLockEnabled:YES];
                    [cropVc setAspectRatioPreset:TOCropViewControllerAspectRatioPresetSquare];
...
{
    [self didGetAllPhotos:@[image] assets:@[cropViewController.asset] infoArr:nil];
}
...
- (void)refreshBottomToolBarStatus {
    ...
    _originalPhotoLabel.hidden = (!_originalPhotoButton.isSelected);
    if (_isSelectOriginalPhoto) [self getSelectedPhotoBytes];
    [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithCustomView:_completeBtn]];
}
